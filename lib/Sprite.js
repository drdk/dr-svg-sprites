...
Sprite.prototype.addSize = function (label) {
	...
	var useLegacyClassName = true;
	var selectorGenerator = util.makeClassName;
	if (config.selector) {
		useLegacyClassName = false;
		switch (typeof config.selector) {
			case "function":
				selectorGenerator = config.selector;
				break;
			case "string":
				selectorGenerator = function (tokens) {
					return util.substitute(config.selector, tokens);
				}
				break;
			default:
				break;
	...
	var items = this.items.map(function (item, index) {
		...
		if (!tokens.name) {
			console.log("no name", index, item);
		}
		var selector = selectorGenerator(tokens);
		return {
			selector: useLegacyClassName ? null : selector,
			className: useLegacyClassName ? selector : null,
