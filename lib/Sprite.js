...
function Sprite (_config) {
	...
	*/
	...
	if ("cssPngPrefix" in config) {
		...
		if (!("cssSvgPrefix" in config) && !("placeholder" in config)) {
	...
	if (config.cssPngPrefix && config.cssPngPrefix.slice(-1) != " ") {
		if(config.placeholder){
			config.cssPngPrefix = config.placeholder + config.cssPngPrefix;
		}
		else {
			config.cssPngPrefix += " ";
		}
	...
	if (config.cssSvgPrefix && config.cssSvgPrefix.slice(-1) != " ") {
		if(config.placeholder){
			config.cssSvgPrefix = config.placeholder + config.cssSvgPrefix;
		}
		else {
			config.cssSvgPrefix += " ";
		}
...
Sprite.prototype.addSize = function (label) {
	...
	var items = this.items.map(function (item) {
		return {
			className: util.makeClassName(item.classNameBase, label, config.prefix, config.placeholder, config.cssSvgPrefix, config.cssPngPrefix),
